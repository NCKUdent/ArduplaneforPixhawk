...
int32_t AP_RollController::_custom_get_rate_out(float desired_rate, bool disable_integrator)
{
	...
	return constrain_float(_custom_last_out_deg * 500, -4500,4500 );
}
...
int32_t AP_RollController::custom_get_servo_out(int32_t angle_err, bool disable_integrator)
{
	...
	float outter_P = 1;
	float angle_err_rad = ToRad((angle_err)/500);
...
int32_t AP_RollController::_track_get_rate_out(float desired_rate, bool disable_integrator)
{
	...
	// Convert to centi-degrees and constrain, beware for physical system constraints
	return constrain_float(_track_last_out_deg * 500, -4500, 4500);
}
